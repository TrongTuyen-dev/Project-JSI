...
function renderWallet(w) {
  ...
  listEl.innerHTML = txs.map(tx => {
    const isDebit   = tx.type === "purchase" || tx.amount < 0;
    const absAmt    = Math.abs(tx.amount);
    const sign      = isDebit ? "−" : "+";
    const color     = isDebit ? "#c0392b" : "#27ae60";
    const iconClass = isDebit ? "bi bi-arrow-up-circle-fill" : "bi bi-arrow-down-circle-fill";
    const iconColor = isDebit ? "#c0392b" : "#c5a059";
    return `
    <div class="tx-item">
      <div class="d-flex align-items-center">
        <div class="tx-icon"><i class="${iconClass}" style="color:${iconColor}"></i></div>
        <div>
          <div class="tx-note">${tx.note || "Giao dịch"}</div>
          <div class="tx-time">${tx.timeStr || fmtTime(tx.time)}</div>
        </div>
      </div>
      <div class="tx-amount" style="color:${color};font-weight:700;">${sign}${fmt(absAmt)}</div>
    </div>`;
  }).join("");
}
